...
let project = "SoccerTypeProvider"
// Short summary of the project
// (used as description in AssemblyInfo and as a short summary for NuGet package)
let summary = "type provider for football teams & games"
// Longer description of the project
// (used as a description for NuGet package; line breaks are automatically cleaned up)
let description = "Project has no description; update build.fsx"
// List of author names (for NuGet package)
let authors = [ "Chris Norris" ]
// Tags for your project (for NuGet package)
let tags = ""
// File system information
let solutionFile = "SoccerTypeProvider.sln"
// Pattern specifying assemblies to be tested using NUnit
let testAssemblies = "tests/**/bin/Release/*Tests*.dll"
// Git configuration (used for publishing documentation in gh-pages branch)
// The profile where the project is posted
let gitOwner = "chrisnorris"
let gitHome = "https://github.com/" + gitOwner
// The name of the project on GitHub
let gitName = "SoccerTypeProvider"
// The url for the raw files hosted
let gitRaw = environVarOrDefault "gitRaw" "https://raw.github.com/chrisnorris"
// --------------------------------------------------------------------------------------
...
Target "AssemblyInfo" (fun _ ->
    ...
    |> Seq.iter (fun (projFileName, projectName, folderName, attributes) ->
           match projFileName with
           | Fsproj ->
               CreateFSharpAssemblyInfo (folderName </> "AssemblyInfo.fs")
                   attributes
           | Csproj ->
...
Target "PublishNuget"
    (fun _ -> Paket.Push(fun p -> { p with WorkingDir = "bin" }))
...
let fakeStartInfo script workingDirectory args fsiargs environmentVars =
    ...
    info.Arguments <- sprintf "%s --fsiargs -d:FAKE %s \"%s\"" args fsiargs
                          script
...
let buildDocumentationTarget fsiargs target =
    trace
        (sprintf
             "Building documentation (%s), this could take some time, please wait..."
             target)
    let exit =
        executeFAKEWithOutput "docs/tools" "generate.fsx" fsiargs
            [ "target", target ]
    if exit <> 0 then failwith "generating reference documentation failed"
    ()

Target "GenerateReferenceDocs"
    (fun _ -> buildDocumentationTarget "-d:RELEASE -d:REFERENCE" "Default")
...
Target "KeepRunning" (fun _ ->
    use watcher =
        !!"docs/content/**/*.*"
        |> WatchChanges(fun changes -> generateHelp' true true)
...
Target "ReleaseDocs" (fun _ ->
    ...
    Repository.cloneSingleBranch "" (gitHome + "/" + gitName + ".git")
        "gh-pages" tempDocsDir
...
Target "Release" (fun _ ->
    ...
    let remote =
        ...
        |> Seq.tryFind
               (fun (s : string) -> s.Contains(gitOwner + "/" + gitName))
        |> function
...
"Clean" ==> "AssemblyInfo" ==> "Build" ==> "CopyBinaries" ==> "RunTests"
==> "GenerateReferenceDocs" ==> "GenerateDocs" ==> "All"
...
"ReleaseDocs" ==> "Release"
"BuildPackage" ==> "PublishNuget" ==> "Release"
